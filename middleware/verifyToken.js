        ...
        if (token == '') {
            ...
            jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, { ignoreExpiration: true }, async (err, decoded) => {
                if (err) {
                    ...
                    req.server_ssh_host = hosting.respond.data.ssh_host
                    req.server_ssh_user = hosting.respond.data.ssh_user
                    req.server_ssh_pass = hosting.respond.data.ssh_password
